...
`LAUNCH_RUNBOOK.md` describes the program deploy step. To ship verified:

- Run `solana-verify build` instead of a plain `anchor build`.
- Deploy that exact artifact.
- Run the verify steps above AFTER the public repo + commit exist.
...
onchain program does not change, only the verification record is
added.
