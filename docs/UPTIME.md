...
What follows does all three. The biggest known footgun, a rebuild in
place with no rollback that can 502 the site, is now structurally
impossible. One honest gap remains (a single VPS); it is called out
below with the fix.
...
| **The active Node process crashes / OOMs** | Caddy active health-checks BOTH instances (`lb_policy first`); on a failed probe it routes to the hot spare within ~1 health interval (3s). systemd `Restart=always` brings the crashed one back. | ~0 to 3s of possible errors for in-flight requests at the instant of the crash |
